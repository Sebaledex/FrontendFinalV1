...
//         console.log('Access token or user not available');
//         return false;
//       }
...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  login: async (username: string, password: string) => {
    ...
    await StorageAdapter.setItem('token', resp.access_token);
    // Log para verificar el usuario devuelto por el login
    console.log('Login successful. User:', resp.user);
    set({ status: 'authenticated', access_token: resp.access_token, user: resp.user });
    return true;
  },
  ...
  signup: async (name: string, username: string, email: string, password: string) => {
    ...
    // Log para verificar el usuario devuelto por el signup
    console.log('Signup successful. User:', resp.user);
    set({ status: 'authenticated', access_token: resp.access_token, user: resp.user });
    return true;
  },
  ...
  logout: async () => {
    await StorageAdapter.removeItem('access_token');
    // Log para verificar que se realizó el logout
